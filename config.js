/** Wedding data & invitation copy. Edit when you add photos or GIFs. */
window.WEDDING_CONFIG = {
  groom: "مهاب",
  bride: "همس",
  groomEn: "Mohab",
  brideEn: "Hams",
  date: "2026-09-30T17:00:00+03:00",
  venue: {
    nameEn: "La Terrace, JW Marriott",
    areaEn: "Fifth Settlement, New Cairo",
    addressEn: "JW Marriott Hotel Cairo, Ring Road",
    mapsUrl:
      "https://www.google.com/maps/place/Club+House/@30.07078,31.4359958,18z/data=!4m6!3m5!1s0x145817197b9f1de9:0x1bf309708d796b1f!8m2!3d30.0699596!4d31.4344754!16s%2Fg%2F11vdbcj_fv?entry=ttu&g_ep=EgoyMDI2MDUyNi4wIKXMDSoASAFQAw%3D%3D",
  },
  eventEn: {
    dateLine: "Wednesday · 30 September 2026",
    dateDetail: "30 / 9 / 2026",
    time: "5:00 PM",
  },
  eventAr: {
    dateLine: "الأربعاء · ٣٠ سبتمبر ٢٠٢٦",
    dateDetail: "٣٠ / ٩ / ٢٠٢٦",
    time: "٥:٠٠ مساءً",
  },
  /** Always shown in Arabic on both invitation pages. */
  quranVerse: {
    text: "وَمِنْ آيَاتِهِ أَنْ خَلَقَ لَكُم مِّنْ أَنفُسِكُمْ أَزْوَاجًا لِّتَسْكُنُوا إِلَيْهَا وَجَعَلَ بَيْنَكُم مَّوَدَّةً وَرَحْمَةً",
    ref: "الروم · ٣٠:٢١",
  },
  /** Soft theme ambiance (SVG — no pixelation). */
  themeArt: {
    light: "assets/backgrounds/garden-light.svg",
    dark: "assets/backgrounds/evening-dark.svg",
  },
  photos: [
    "Photo1.jpeg",
    "Photo2.jpeg",
    "Photo 3.jpeg",
    "Photo4.jpeg",
    "Photo5.jpeg",
    "Photo6.jpeg",
    "Photo7.jpeg",
    "Photo8.jpeg",
    "Photo9.jpeg",
    "Photo10.jpeg",
    "Photo11.jpeg",
  ],
  /** Background music per invitation language. */
  audio: {
    en: "assets/audio/sky-full-of-stars.mp3",
    ar: "assets/audio/wayah.mp3",
  },
  /**
   * RSVP backend — use **one** of these:
   *
   * A) Supabase (recommended if Apps Script fails) — see supabase/README.md
   * B) Google Sheet via Apps Script — see google-apps-script/README.md
   */
  rsvp: {
    scriptUrl: "https://script.google.com/macros/s/AKfycbykrlIW-JyHpB4J98qnr8oClQYGx1y8X6v1lotYiBohTUrB2DiQDnWg7jxvM42qy73D/exec",
    supabase: {
      url: "https://huwqarxxdvtnbebxjglx.supabase.co",
      anonKey: "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Imh1d3Fhcnh4ZHZ0bmJlYnhqZ2x4Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODA1MTU2NzMsImV4cCI6MjA5NjA5MTY3M30.g1sBs-dzdJGXsmNjRP35DGhEKJokibAa3ORLTKRNTh0",
      table: "rsvps",
    },
  },
};

window.INVITATION_COPY = {
  ar: {
    lang: "ar",
    dir: "rtl",
    documentTitle: "Hams&Mohab Wedding",
    metaDescription: "أنتم مدعوون إلى حفل زفاف Hams&Mohab — ٣٠ سبتمبر ٢٠٢٦",
    eyebrow: "💌 ندعوكم لمشاركتنا فرحتنا",
    tagline: "",
    lyricsLines: [
      "وياه الحياة هتحلى وأنا معاه",
      "هو دا اللي أنا بتمناه واللي عيني شايفاه",
      "إحساس إنه أحلى وأغلى الناس",
      "خلى قلبي يقول لي خلاص إهدى بقى لقينا",
    ],
    footerLine: "🎉 بانتظاركم بفرحنا",
    footerEmoji: "💕",
    footerNamesJoin: " و ",
    photoAltPrefix: "ذكرى",
    rsvpTitle: "💌 تأكيد الحضور",
    rsvpLead: "يسعدنا معرفة إن كنتم ستشاركونا فرحتنا",
    rsvpName: "الاسم",
    rsvpAttending: "هل ستحضرون؟",
    rsvpAttendingYes: "نعم، بكل سرور!",
    rsvpAttendingNo: "للأسف لا أستطيع",
    rsvpMessage: "رسالة (اختياري)",
    rsvpSubmit: "إرسال التأكيد",
  },
  en: {
    lang: "en",
    dir: "ltr",
    documentTitle: "Hams&Mohab Wedding",
    metaDescription: "You're invited to Hams&Mohab Wedding — 30 September 2026",
    eyebrow: "💌 We invite you to share our joy",
    tagline: "✨ Together we begin a new chapter",
    lyricsLines: [
      "Cause you're a sky, cause you're a sky full of stars",
      "I'm gonna give you my heart",
      "Cause you're a sky, cause you're a sky full of stars",
      "Cause you light up the path",
    ],
    footerLine: "🎉 We cannot wait to celebrate with you",
    footerEmoji: "💕",
    footerNamesJoin: " & ",
    photoAltPrefix: "Memory",
    rsvpTitle: "💌 RSVP",
    rsvpLead: "We would love to know if you can celebrate with us",
    rsvpName: "Your name",
    rsvpAttending: "Will you attend?",
    rsvpAttendingYes: "Yes, joyfully!",
    rsvpAttendingNo: "Sorry, I can't make it",
    rsvpMessage: "Message (optional)",
    rsvpSubmit: "Send RSVP",
  },
};

window.UI_COPY = {
  countdownTitle: "⏳ Countdown",
  days: "Days",
  hours: "Hours",
  minutes: "Minutes",
  seconds: "Seconds",
  detailsTitle: "📍 Celebration details",
  labelDate: "📅 Date",
  labelTime: "🕕 Time",
  labelVenue: "🏛️ Venue",
  openMaps: "🗺️ Open location in Maps",
  galleryTitle: "💖 Our memories",
  galleryLead: "A little stack of our favorite moments",
  stackLabelLeft: "💕",
  stackLabelRight: "✨",
  backToTop: "⬆️ Back to top",
  rsvpSending: "Sending your RSVP…",
  rsvpSuccess: "Thank you! Your RSVP was received. 💕",
  rsvpError: "Something went wrong. Please try again.",
  rsvpNotConfigured:
    "RSVP is not connected yet. Please contact the couple directly.",
  themeLight: "Switch to light mode",
  themeDark: "Switch to dark mode",
  audioMute: "Mute music",
  audioUnmute: "Unmute music",
  scrollHint1: "Scroll to celebrate",
  scrollHint2: "Our memories await",
  dualLyricsTitleAr: "🎵 أغنيتنا",
  dualLyricsTitleEn: "🎵 Our song",
};

window.UI_COPY_AR = {
  countdownTitle: "⏳ العد التنازلي",
  days: "يوم",
  hours: "ساعة",
  minutes: "دقيقة",
  seconds: "ثانية",
  detailsTitle: "📍 تفاصيل الاحتفال",
  labelDate: "📅 التاريخ",
  labelTime: "🕕 الموعد",
  labelVenue: "🏛️ المكان",
  openMaps: "🗺️ الموقع على الخريطة",
  galleryTitle: "💖 ذكرياتنا",
  galleryLead: "لمحات من أجمل لحظاتنا",
  stackLabelLeft: "💕",
  stackLabelRight: "✨",
  backToTop: "⬆️ العودة للأعلى",
  rsvpSending: "جاري إرسال التأكيد…",
  rsvpSuccess: "شكراً لكم! تم استلام تأكيدكم. 💕",
  rsvpError: "حدث خطأ. يرجى المحاولة مرة أخرى.",
  rsvpNotConfigured: "التأكيد غير متصل بعد. يرجى التواصل معنا مباشرة.",
  themeLight: "التبديل إلى الوضع الفاتح",
  themeDark: "التبديل إلى الوضع الداكن",
  audioMute: "كتم الصوت",
  audioUnmute: "تشغيل الصوت",
  scrollHint1: "مرّر للاحتفال",
  scrollHint2: "ذكرياتنا بانتظاركم",
  dualLyricsTitleAr: "🎵 أغنيتنا",
  dualLyricsTitleEn: "🎵 Our song",
};
